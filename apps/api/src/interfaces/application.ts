...
import { audioWsRouter } from "./router/audio-ws.router.ts";
import { companyRouter } from "./router/company.router.ts";
import { generateRouter } from "./router/generate.router.ts";
...
import { whatsappRouter } from "./router/whatsapp.router.ts";
import { whatsappWebhookRouter } from "./router/whatsapp-webhook.router.ts";
...
// WhatsApp webhook routes (public — Meta webhooks cannot authenticate with session cookies)
app.route("/api/whatsapp", whatsappWebhookRouter);
...
app.route("/api/v1/company", whatsappRouter);
app.route("/api/v1/room", audioWsRouter);
