...
type ApiWebSocketEventCallback<T extends ApiWebSocketEventEnum> = (data: ApiWebSocketEventType[T]) => void;

export class ApiWebSocket {
  private socket: WebSocket | null = null;
  // Map of event name to Set of callbacks
  private eventListeners: Map<string, Set<ApiWebSocketEventCallback<ApiWebSocketEventEnum>>> = new Map();
  // Map for once listeners to allow removal after first call
  private onceListeners: Map<string, Set<ApiWebSocketEventCallback<ApiWebSocketEventEnum>>> = new Map();
  ...
  constructor(private readonly path: string = "/ws") {
    ...
    this.socket.addEventListener("open", (event) => this.dispatch(ApiWebSocketEventEnum.OPEN, event));
    this.socket.addEventListener("close", (event) => this.dispatch(ApiWebSocketEventEnum.CLOSE, event));
    this.socket.addEventListener("error", (event) => this.dispatch(ApiWebSocketEventEnum.ERROR, event));
    this.socket.addEventListener("message", (event) => this.dispatch(ApiWebSocketEventEnum.MESSAGE, event));
  }

  private dispatch<T extends ApiWebSocketEventEnum>(eventName: ApiWebSocketEventEnum, event: ApiWebSocketEventType[T]) {
    // Regular listeners
  ...
  on<T extends ApiWebSocketEventEnum>(event: T, callback: ApiWebSocketEventCallback<T>) {
    if (!this.socket) {
    ...
    this.eventListeners.get(event)!.add(callback as ApiWebSocketEventCallback<ApiWebSocketEventEnum>);
  }
