...
export function useRealtimeCall(audioRef: React.RefObject<HTMLAudioElement>) {
  ...
  const start = async (companyId: string) => send({ type: RealtimeCallMachineEvent.START, companyId, audioRef })
  const stop = () => send({ type: RealtimeCallMachineEvent.STOP })
  const sendMessage = (message: string) => send({ type: RealtimeCallMachineEvent.MESSAGE, message })
